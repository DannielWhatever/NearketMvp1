...
angular.module('hackaton700.controllers').controller('TheMapController', function ($scope, $stateParams, $cordovaGeolocation, ItemFactory, BusquedaService) {
  ...
  function loadItems(itemId, advancedSearch) {
    ItemFactory.buscar(itemId, 'TUxDQ1NBTjk4M2M', advancedSearch).then(function (resp) {
      console.log(resp);
      ...
      _.forEach(results, function (item) {
        ...
        new google.maps.event.addListenerOnce($scope.map, 'idle', function () {
          ...
          '<h3>' + item.title + '</h3>' + '<p>' + '<b>$' + item.price + '</b><br/>' + '<span>Disponibles ' + item.available_quantity + ' / Vendidos ' + item.sold_quantity + '</span><br/>' + '<span>' + (item.condition == 'new' ? 'Nuevo' : 'Usado') + '</span><br/>' + '</p>' + '<a href="' + item.permalink + '" class="button button-energized">Compra en ML</a>' + '</div>' + '</div>';
          ...
          $scope.map.addListener('toogleOff' + item.category, function () {
